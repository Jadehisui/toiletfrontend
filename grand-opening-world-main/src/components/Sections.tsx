...
export function Tokenomics() {
  ...
  const ca = "0x000000000000000000000000DEADBEEF";

  const stats = [
    { label: "Supply", value: "1,000,000,000" },
    { label: "Tax", value: "0% / 0%" },
    { label: "LP", value: "BURNED 🔥" },
    { label: "Owner", value: "RENOUNCED" },
  ];
    ...
    <section id="tokenomics" className="relative overflow-hidden bg-[var(--gta-night)] py-20 md:py-28">
      ...
      <div className="relative mx-auto max-w-6xl px-5">
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="mx-auto mt-8 max-w-3xl border-4 border-black bg-[var(--gta-green)] p-1"
        >
          <div className="flex items-center justify-between gap-3 bg-black px-4 py-3">
            <div className="overflow-hidden">
              <p className="font-stencil text-[10px] tracking-widest text-white/50">
                CONTRACT
              </p>
              <p className="truncate font-mono text-xs text-[var(--gta-green)] md:text-sm">
                {ca}
              </p>
            </div>
            <button
              ...
              className="shrink-0 border-2 border-black bg-[var(--gta-green)] px-3 py-2 font-stencil text-[10px] uppercase tracking-widest text-black hover:bg-[var(--gta-cream)]"
            >
              {copied ? "Copied" : "Copy"}
            </button>
          </div>
...
export function TheCrew() {
  const socials = [
    ...
    { name: "DexScreener", href: "#", icon: "📈" },
    { name: "Chart", href: "#", icon: "◢◤" },
  ];
  return (
    <section
      id="crew"
